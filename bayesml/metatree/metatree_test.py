...
gen_model = metatree.GenModel(
    ...
    sub_h_params={'h_lambda_mat':np.eye(dim_continuous)*0.01},
    seed=2,
    c_num_assignment_vec=np.array([-1,0]),
    c_num_children_vec=2,
    SubModel=linearregression,
)
x_continuous = np.random.rand(100,2)*6 - 3
x_continuous[:,1] = 1
gen_model.gen_params()
gen_model.visualize_model(filename='tree.gv',x_continuous=x_continuous)
