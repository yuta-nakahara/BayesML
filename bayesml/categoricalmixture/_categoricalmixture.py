...
def float_mat_sum_1(val, val_name, exception_class):
    if type(val) is np.ndarray:
        if np.issubdtype(val.dtype,
                         np.integer) and val.ndim == 2 and np.all(np.abs(np.sum(val, axis=1) - 1.) <= _EPSILON):
            return val.astype(float)
        if np.issubdtype(val.dtype,
                         np.floating) and val.ndim == 2 and np.all(np.abs(np.sum(val, axis=1) - 1.) <= _EPSILON):
            return val
    raise (exception_class(
        val_name + " must be a 2-dimensional numpy.ndarray, and the sum of the second dimension must equal to 1."))
...
class GenModel(base.Generative):
    ...
    def __init__(
        ...
        *,
        ...
        if (self.m_degree != self.pi_vec.shape[0] or self.m_degree != self.h_alpha_vec.shape[0]):
        ...
        if (self.degree != self.theta_mat.shape[1] or self.degree != self.h_beta_vec.shape[0]):
            raise (ParameterFormatError("degree of the second dimension of theta_mat" +
    ...
    def set_h_params(self, h_alpha_vec, h_beta_vec):
        ...
        if self.m_degree != self.pi_vec.shape[0]:
            ...
            warnings.warn(
                "pi_vec is reinitialized to [1.0/self.m_degree, 1.0/self.m_degree, ... , 1.0/self.m_degree] \
                     because dimension of pi_vec and h_alpha_vec are mismatched.", ParameterFormatWarning)

        if self.m_degree != self.theta_mat.shape[0]:
            self.theta_mat = np.ones((self.m_degree, self.degree)) / self.degree
            warnings.warn(
                "theta_mat is reinitialized to \
                    [[1.0/self.degree, 1.0/self.degree, ... , 1.0/self.degree]]*self.m_degree \
                     because dimension of theta_mat and h_alpha_vec are mismatched.", ParameterFormatWarning)
        elif self.degree != self.theta_mat.shape[1]:
            self.theta_mat = np.ones((self.m_degree, self.degree)) / self.degree
            warnings.warn(
                "theta_mat is reinitialized to \
                    [[1.0/self.degree, 1.0/self.degree, ... , 1.0/self.degree]]*self.m_degree  \
                    because dimension of theta_mat and h_beta_vec are mismatched.", ParameterFormatWarning)
        ...
        if self.degree != self.h_beta_vec.shape[0]:
            ...
            warnings.warn(
                "h_beta_vec is reinitialized to [1/2, 1/2, ..., 1/2] \
                    because dimension of h_beta_vec and theta_mat are mismatched.", ParameterFormatWarning)
