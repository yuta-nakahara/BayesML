...
r"""
The categorical mixture model with the Dirichlet prior distributions. 
...
* :math:`\boldsymbol{x} \in \{ 0, 1\}^d`: a data point, (a one-hot vector, i.e., :math:`\sum_{l=1}^d x_l=1`)
...
* :math:`\boldsymbol{\theta}_{\mathrm{p},k}=(\theta_{\mathrm{p},k,1},\theta_{\mathrm{p},k,2},\cdots,\theta_{\mathrm{p},k,d})^\top \in [0, 1]^d`: the parameter of the predictive distribution (:math:`\sum_{l=1}^d \theta_{\mathrm{p}, k,l}=1`)
...
from ._categoricalmixture import GenModel
